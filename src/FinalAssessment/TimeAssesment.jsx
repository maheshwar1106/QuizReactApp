...
const TimeAssesment = ({ tTime, rTime }) => {
  const data = {
    labels: ["Total Time", "Submit Time"],
    datasets: [
      {
        label: "Time(sec)",
        data: [`${tTime}`, `${tTime - rTime}`, `${tTime + 10}`],
  ...
  return (
    ...
    >
      <h2 style={{ margin: "30px 0px" }}>Time Chart</h2>
      <Bar width={140} height={250} data={data} options={options} />
    </div>
  );
};

export default TimeAssesment;
